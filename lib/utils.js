...
export function cloneLayoutItem(layoutItem: LayoutItem): LayoutItem {
  return {
    ...
    isGridLayout: layoutItem.isGridLayout,
    gridLayoutId: layoutItem.gridLayoutId,
...
export function getFirstCollision(
  ...
  for (let i = 0, len = layout.length; i < len; i++) {
    const isCollided = collides(layout[i], layoutItem);
    if (isCollided) return layout[i];
  }
}

export function getContainerGridLayout(
  layout: Layout,
  layoutItem: LayoutItem
): ?LayoutItem {
  ...
  for (let i = 0; i < len; i++) {
    ...
    if (isL1InsideL2(layoutItem, item) && (!insideL || item.w < insideL.w))
      insideL = item;
  }
  if (insideL) {
    layoutItem.gridLayoutId = insideL.i;
  } else {
    layoutItem.gridLayoutId = null;
  }
  return insideL;
...
function isL1InsideL2(l1: LayoutItem, l2: LayoutItem) {
  if (l1.i === l2.i || l2.isGridLayout !== true) return false;
  if (
    l2.x + l2.w >= l1.x + l1.w &&
    l2.x <= l1.x &&
    l2.y <= l1.y &&
...
export function moveElement(
  ...
  if (collisions.length && (collisions[0].isGridLayout || l.isGridLayout)) {
    return layout;
  }
...
export function synchronizeLayoutWithChildren(
  ...
  React.Children.forEach(children, (child: ReactElement<any>, i: number) => {
    ...
    if (exists) {
      ...
      console.log("notExists", child.key, child.props.grid, child);
